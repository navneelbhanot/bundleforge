...
export function capWarningTemplate(args: CapWarningArgs): EmailTemplate {
  ...
  const subject = `${shopName}: ${count} of ${cap} MintBundle orders used this month`;

  const text =
    `Hi ${shopName},\n` +
    `\n` +
    `Heads up — your shop has used ${count} of its ${cap} monthly bundle\n` +
    `orders on the MintBundle Starter plan. That's about ${remaining} bundle\n` +
  ...
  const html =
    ...
    `<p>Heads up — your shop has used <strong>${count} of its ${cap} monthly bundle orders</strong> on the MintBundle Starter plan.</p>` +
